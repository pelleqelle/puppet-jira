      ...
      on_supported_os.each do |os, facts|
        context "on #{os} #{facts}" do
          ...
          context 'default params' do
            ...
            it { expected.to contain_group('jira') }
            it { expected.to contain_user('jira').with_shell('/bin/true') }
            it 'deploys jira 6.3.4a from tar.gz' do
              expected.to contain_archive('/tmp/atlassian-jira-6.3.4a.tar.gz').
            ...
            it 'manages the jira home directory' do
              expected.to contain_file('/home/jira').with('ensure' => 'directory',
                                                     'owner' => 'jira',
                                                     'group' => 'jira')
            end
          end
          ...
          context 'jira 7' do
            context 'default product' do
              ...
              it 'deploys jira 7.0.4 from tar.gz' do
                expected.to contain_archive('/tmp/atlassian-jira-software-7.0.4-jira-7.0.4.tar.gz').
            ...
            context 'core product' do
              ...
              it 'deploys jira 7.0.4 from tar.gz' do
                expected.to contain_archive('/tmp/atlassian-jira-core-7.0.4.tar.gz').
          ...
          context 'overwriting params' do
            ...
            it do
              expected.to contain_user('foo').with('home' => '/random/homedir',
                                              'shell' => '/bin/bash',
                                              'uid'   => 333,
                                              'gid'   => 444)
            end
            it { expected.to contain_group('bar') }

            it 'deploys jira 6.0.0 from tar.gz' do
              expected.to contain_archive('/tmp/atlassian-jira-6.0.0.tar.gz').
            ...
            it 'manages the jira home directory' do
              expected.to contain_file('/random/homedir').with('ensure' => 'directory',
                                                          'owner' => 'foo',
                                                          'group' => 'bar')
            end
          end
