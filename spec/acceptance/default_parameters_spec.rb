...
describe 'jira postgresql', unless: UNSUPPORTED_PLATFORMS.include?(fact('osfamily')) do
  ...
  describe service('jira') do
    it { expected.to be_enabled }
    it { expected.to be_running }
  end

  describe user('jira') do
    it { expected.to exist }
  end

  describe user('jira') do
    it { expected.to belong_to_group 'jira' }
  end

  describe user('jira') do
    it { expected.to have_login_shell '/bin/true' }
  end

  describe command('wget -q --tries=240 --retry-connrefused --read-timeout=10 -O- localhost:8080') do
    its(:stdout) { expected.to match(%r{6\.2\.7}) }
  end
